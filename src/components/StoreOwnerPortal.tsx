...
export const StoreOwnerPortal: React.FC<StoreOwnerPortalProps> = ({
  ...
  const handleToggleWakeLock = async () => {
    if (wakeLockActive) {
      ...
      if (ok) {
        showSystemNotification("وضع الشاشة المتيقظة ☀️", {
          body: "ستبقى الشاشة نشطة ولن تنطفئ لضمان رنين كل طلب وارد فوراً!",
          soundType: "chime"
        });
      }
    }
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border border-slate-700/80 rounded-3xl p-4 sm:p-7 text-white shadow-xl space-y-4 sm:space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3.5 sm:gap-4">
          ...
          <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-800">
            ...
            {/* Screen WakeLock button so phone doesn't sleep while waiting for orders */}
            {isWakeLockSupported() && (
              <button
                type="button"
                ...
                title={wakeLockActive ? "الشاشة متيقظة ولن تنطفئ (انقر للتعطيل)" : "إبقاء الشاشة مفعلة لتلقي الطلبات فوراً دون نوم الهاتف"}
              >
