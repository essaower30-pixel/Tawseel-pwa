...
import { Bell, Volume2, X, CheckCircle2, Smartphone, HelpCircle } from "lucide-react";
  ...
  const handleEnable = async () => {
    ...
    try {
      ...
      if (granted) {
        ...
        // Hide after showing success state for 3 seconds
        setTimeout(() => {
          setIsVisible(false);
        }, 3200);
      } else {
          ...
          <div className="bg-slate-900/95 backdrop-blur-md border-2 border-orange-500/80 text-white p-4 rounded-2xl shadow-2xl shadow-orange-950/40 relative overflow-hidden">
            ...
            {justGranted ? (
              ...
              <div className="space-y-3">
                ...
                {/* Android Sound Help trigger */}
                <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between text-[11px]">
                  <span className="text-slate-400 flex items-center gap-1">
                    <Smartphone className="w-3.5 h-3.5 text-orange-400" />
                    أندرويد: يظهر رقم فقط بدون رنين؟
                  </span>
                  <button
                    type="button"
                    onClick={() => setShowAndroidHelp(true)}
                    className="text-orange-400 hover:text-orange-300 font-bold hover:underline flex items-center gap-1"
                  >
